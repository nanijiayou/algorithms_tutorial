...
2. 求最优方案数量:  
修改dp定义，$f_{i,j}$ 为在只能放前 $i$ 个物品的情况下，容量为 $j$ 的背包 「正好装满」 所能达到的最大总价值。  修改之后，每种DP状态都可以用一个 $g_{i,j}$ 来表示方案数。  
     $f_{i,j}$ 表示只考虑前 $i$ 个物品时背包体积「正好」是 $j$ 时的最大价值
     $g_{i,j}$ 表示只考虑前 $i$ 个物品时背包体积「正好」是 $j$ 时的方案数。

    **转移方程为**:  

    1. 如果 $f_{i,j} = f_{i-1,j}$ 且 $f_{i,j} \neq f_{i-1,j-v}+w$，此时不选择把物品装入背包更优，方案数由 $g_{i-1,j}$ 转移过来。

    2. 如果 $f_{i,j} \neq f_{i-1,j}$ 且 $f_{i,j} = f_{i-1,j-v}+w$，此时选择把物品装入背包更优，方案数由 $g_{i-1,j-v}$ 转移过来。

    3. 如果 $f_{i,j} = f_{i-1,j}$ 且 $f_{i,j} = f_{i-1,j-v}+w$，此时选择把物品装入或者不放入都取得最优，方案数由 $g_{i-1,j-v}$ 转移过来。
    ...
    总结：不管是从 $f_{i-1,j}$ 还是 $f_{i-1, j-v}$ 转移过来，对于方案数来说，都是可行的，所以需要更新方案数。
...
```
[最优求方案数](https://www.acwing.com/problem/content/description/11/)
